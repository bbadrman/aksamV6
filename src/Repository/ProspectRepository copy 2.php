...
class ProspectRepository extends ServiceEntityRepository
{
    ...
    public function findAvenir(SearchProspect $search): PaginationInterface
    {
        ...
        if (!empty($search->l)) {
            $query = $query
                ->andWhere('p.phone LIKE :l OR p.gsm LIKE :l')
                ->setParameter('l', "%{$search->l}%");
        }
    ...
    public function findRelancesNonTraitees(SearchProspect $search): PaginationInterface
    {
        ...
        $query = $this->createQueryBuilder('p')
            ...
            ->leftJoin('p.team', 't')
            ->leftJoin('p.comrcl', 'f')
            ->leftJoin('p.relanceds', 'r', 'WITH', 'r.motifRelanced = 1 AND r.relacedAt > :dayBeforeYesterday')
            ->where('p.comrcl is NOT NULL')
            ->andWhere('p.team is NOT NULL')
            ->andWhere('p.id NOT IN (SELECT pr.id FROM App\Entity\Prospect pr JOIN pr.relanceds rel WHERE rel.relacedAt > :yesterday)')

            ->andWhere('p.id NOT IN (SELECT p2.id FROM App\Entity\Prospect p2 JOIN p2.relanceds r2 WHERE r2.relacedAt > :yesterday)')
            ->setParameter('dayBeforeYesterday', $yesterday->modify('-1 year'))
            ->setParameter('yesterday', $yesterday);

        // Apply search filters
        if (!empty($search->q)) {
            $query->andWhere('p.name LIKE :q')->setParameter('q', "%{$search->q}%");
        }
        // ... Add similar logic for other search filters ...
